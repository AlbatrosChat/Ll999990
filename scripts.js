...
function register(email, password) {
    ...
    const newUser = { id: database.users.length + 1, email, password, role: 'user' };
...
document.querySelector('#login-form')?.addEventListener('submit', (e) => {
    ...
    if (login(email, password)) {
        ...
        window.location.href = 'index.html'; // إعادة التوجيه إلى الصفحة الرئيسية بعد تسجيل الدخول
    } else {
...
document.querySelector('#register-form')?.addEventListener('submit', (e) => {
    ...
    if (register(email, password)) {
        window.location.href = 'login.html'; // إعادة التوجيه إلى صفحة تسجيل الدخول بعد التسجيل
    }
});
